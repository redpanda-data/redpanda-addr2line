...
If you have `docker compose` installed (the legacy `docker-compose` binary
works too) then you can run the application locally using the provided
script (add `--docker` if not using `podman`):
