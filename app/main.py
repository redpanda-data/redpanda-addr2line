...
def versions():
    ...
    for dir in download_dir.iterdir():
        ...
        if not re.match(r"^\d{2}\.\d\.\d{1,2}$", dir.name):
            continue
        versions.append(dir.name)
    return versions


def redpanda_exec_path(version):
    """
    ...
    """
    redpanda = download_dir / version / "libexec" / "redpanda"
...
def decode_redpanda_backtrace(trace, version):
    """
...
def backtrace(
  ...
  0x1d472a4""",
    ...
    return decode_redpanda_backtrace(trace, version)
